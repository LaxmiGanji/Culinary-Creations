...
function Profile() {
  ...
  if (bioChange === "true") {
    const newBio = event.target["bio-text"].value.trim();
  ...
  if (bioChange === "false") {
    return (
      <button onClick={bioSetting} type="button" className="btn btn-outline-secondary">
        Change Bio
      </button>
    );
  } else {
    return (
      <form onSubmit={bioSetting} action="javascript:void(0);" method="POST" className="bio-form">
        <textarea
        ...
        <div style={{ marginTop: "10px" }}>
          ...
          <button type="button" onClick={() => updateBio("false")} className="btn btn-secondary" style={{ marginLeft: "10px" }}>
            Cancel
          </button>
        </div>
      </form>
