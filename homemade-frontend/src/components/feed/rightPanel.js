...
// List of developer usernames to filter out
const DEVELOPER_USERNAMES = ['admin', 'developer', 'test', 'ankit_0301', 'krish_garg'];
...
function RightPanel() {
  ...
  async function fetchPosts(){
    ...
    // Filter out posts from developer accounts
    const filteredPosts = temp[0].filter(post => !DEVELOPER_USERNAMES.includes(post.username.toLowerCase()));
					...
					<div className="image-user-post col-xs-1 col-sm-1 col-md-1 col-lg-1">
						<div className="custom-file-input-2">
							<label htmlFor="file-input">
								<img style={{height:"50px", width:"75px",cursor: "pointer"}} src={imageImg} alt="Upload"/>
							</label>
