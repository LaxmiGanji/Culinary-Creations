...
def load_dataset(silent=False):
    ...
    for dataset_file_name in dataset_file_names:
        dataset_file_path = os.path.join(os.path.dirname(__file__), 'tmp', 'datasets', dataset_file_name)

        print("Trying to load:", dataset_file_path)
...
def build_model(vocab_size, embedding_dim, rnn_units, batch_size):
    ...
    model.add(tf.keras.Input(batch_size=batch_size, shape=(None,)))
...
filepath_of_trained_model = os.path.join(os.path.dirname(__file__), "Model", "Model.h5")
...
try:
    ...
    print("Successfully loaded model weights from:", filepath_of_trained_model)
except Exception as e:
    print("Error loading model weights:", str(e))
    print("Please ensure the Model directory exists and contains Model.h5 file")
    raise
...
def home():
    try:
        ...
        # Split ingredients into a list if it's a string
        ...
        print("Processing ingredients:", ingredients_list)

        # Generate recipes
        ans = generate_combinations(model_simplified, ingredients_list)
        if not ans or not ans.get(0):
            return jsonify({'error': 'No recipes found for these ingredients'}), 404

        print(f"Generated {len(ans[0])} recipes")
        return jsonify({'result': ans})

    except Exception as e:
        print("Error in ingredient route:", str(e))
